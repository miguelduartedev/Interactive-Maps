...
const CountryShape = memo(function CountryShape({ element, country, hovered }) {
  const color = useSelector((state) => state.mapState.countryColors[country])
...
const Geometry = memo(function Geometry({ tree, hovered }) {
  return tree.map(({ element, country, children }, index) => {
    if (country) return <CountryShape key={index} element={element} country={country} hovered={country === hovered} />
    if (children) return cloneElement(element, {}, <Geometry tree={children} hovered={hovered} />)
    return element
  })
})
...
export default function MapCanvas({ currentMap, children, ...svgProps }) {
  ...
  return (
    <svg {...svgProps} id={currentMap} ref={svgRef}
      ...
      <Geometry tree={geometry.tree} hovered={hovered} />
    </svg>
  )
}
